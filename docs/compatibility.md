...
| `status <id> <status>` | Supported | Semantic | Accepts `open`, `in_progress`, and `closed`. |
...
| `link <id> <id> [id...]` | Partial | Semantic | Currently links two tickets; upstream supports multiple targets. |
| `unlink <id> <target-id>` | Supported | Semantic | Removes symmetric link. |
| `ls` / `list` | Supported | Semantic; stable shape for human output | Also includes closed tickets as an intentional convenience. |
...
| plugin `ls` / `list` | Deferred | To be decided | Core `gtk list` already covers the common list workflow. |
...
CI runs formatting, tests, vet coverage, and build checks across Linux, macOS,
and Windows. Tag-triggered release automation publishes `gtk` archives and
checksums for supported operating systems and architectures.
